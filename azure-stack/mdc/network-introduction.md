...
description: Obtenga información sobre redes para el dispositivo del Centro de datos modular de Azure.
author: BryanLa
ms.author: bryanla
...
ms.locfileid: "94674581"
---
# <a name="modular-datacenter-mdc-network-introduction"></a>Introducción a la red del Centro de datos modular (MDC)

## <a name="package-content"></a>Contenido de paquete

La solución Centro de datos modular (MDC) de Azure se incluye con el equipamiento de red adicional al que se hace referencia en este artículo. Este equipo se usa para conectar el contenedor a la red.
...
La solución MDC requiere una infraestructura física resistente y de alta disponibilidad para admitir sus operaciones y servicios. Los vínculos superiores de la parte superior del rack (TOR) a los conmutadores perimetrales se limitan a medios SFP+ o SFP28 y a velocidades de 1 GB, 10 GB o 40 GB.

En el siguiente diagrama se presenta nuestro diseño recomendado para MDC.

![Diagrama que muestra el diseño de red física recomendado.](media/network-introduction/physical-network-design.png)
...
Todas estas VLAN y subredes están asociadas con la red lógica en cada ubicación física.
...
|---------------------|-----------------|----------|
| Dirección IP virtual (VIP) pública | MDC usa un total de 31 direcciones de esta red. Ocho direcciones IP públicas se utilizan para un pequeño conjunto de servicios de MDC, mientras que el resto las usan las VM del inquilino. Si tiene previsto usar Azure App Service y los proveedores de recursos de SQL, se usan siete direcciones más. Las 15 direcciones IP restantes están reservadas a los futuros servicios de Azure. | /26 (62 hosts) - /22 (1022 hosts) <br><Br>Recomendado = /24 (254 hosts) |
...
| Privados | Se usa para la red de almacenamiento, direcciones IP virtuales privadas, contenedores de infraestructura y otras funciones internas. | /20 |
| Controlador de administración de placa base (BMC) | Se utiliza para comunicarse con los BMC en los hosts físicos. | /26 |
...
La infraestructura de red de MDC consta de varias redes lógicas que están configuradas en los conmutadores. El diagrama siguiente muestra estas redes lógicas y cómo integrarlas con la parte superior del rack (TOR), BMC y los conmutadores perimetrales (red de cliente).

![Diagrama que muestra el diseño de la red lógica.](media/network-introduction/logical-network-design.png)

#### <a name="bmc-network"></a>Red de BMC

Esta red está dedicada a conectar todos los BMC (también conocidos como procesadores de servicio) a la red de administración. Algunos ejemplos son iDRAC, iLO e iBMC. Solo se usa una cuenta de BMC para comunicarse con cualquier nodo de BMC. Si está presente, el host del ciclo de vida del hardware (HLH) se ubica en esta red y puede proporcionar software específico del OEM para el mantenimiento o la supervisión del hardware.

El HLH también hospeda la VM de implementación (DVM). La DVM se utiliza durante la implementación de MDC y se quita cuando esta finaliza. La DVM requiere acceso a Internet en escenarios de implementación conectados para probar, validar y obtener acceso a varios componentes. Estos componentes pueden estar dentro y fuera de la red corporativa. Algunos ejemplos son NTP, el Sistema de nombres de dominio (DNS) y Azure. Para obtener más información acerca de los requisitos de conectividad, consulte la sección de traducción de direcciones de red (NAT) en la integración con el firewall de MDC.
...
- **Red de almacenamiento**. Una red /25 (128 direcciones IP) que se utiliza para admitir el uso del tráfico de almacenamiento de Espacios de almacenamiento directo y el bloque de mensajes del servidor (SMB), así como la migración en vivo de máquinas virtuales.
...
Esta red /29 (seis IP de host) está dedicada a conectar los puertos de administración de los conmutadores. Permite el acceso fuera de banda para la implementación, la administración y la solución de problemas. Se calcula a partir de la red de infraestructura de conmutadores mencionada en la sección anterior.

#### <a name="isilon-network"></a>Red Isilon

Hay dos redes /25. Una reside en el conmutador TOR y otra /25 se usa en el conmutador BMC para la administración.
...
Se le solicitará que proporcione información importante relacionada con DNS cuando implemente la solución de MDC.
...
Los ejemplos de algunos de los puntos de conexión de esta implementación tendrían un aspecto parecido a las direcciones URL siguientes:
...
Para resolver los nombres DNS para los puntos de conexión y las instancias de MDC desde fuera de MDC, debe integrar los servidores DNS. Incluya los servidores que hospedan la zona DNS externa para MDC con los servidores DNS que hospedan la zona principal que quiere usar.
...
Si un inquilino especifica **Myapp** en el campo de la etiqueta de nombre DNS de un recurso de dirección IP pública, crea un registro A para myapp en la zona *east.cloudapp.cloud.fabrikam.com* en el servidor DNS externo de MDC. El FQDN resultante sería *myapp.east.cloudapp.cloud.fabrikam.com*.

Si quiere aprovechar esta funcionalidad y usar este espacio de nombres, debe integrar los servidores DNS. Se incluyen los servidores que hospedan la zona DNS externa para MDC y también los servidores DNS que hospedan la zona principal que quiere usar. Este espacio de nombres es distinto del espacio de nombres utilizado para los puntos de conexión de servicio de MDC, por lo que debe crear una delegación adicional o una regla de reenvío condicional.

Para obtener más información acerca del funcionamiento de la etiqueta de nombre DNS, consulte la sección sobre el uso de DNS de la documentación de MDC.
...
Se recomienda el método NAT para permitir que DVM tenga acceso a los recursos externos durante la implementación. También se recomienda NAT para las VM de la consola de recuperación de emergencia (ERCS) o PEP durante el registro y la solución de problemas.

NAT también puede ser una alternativa a las direcciones IP públicas de la red externa o VIP públicas. No recomendamos esta opción, ya que limita la experiencia del usuario inquilino y aumenta la complejidad. Una opción sería una NAT con una relación de uno a uno que siga requiriendo una dirección IP pública por cada IP de usuario del grupo. Otra opción es una NAT con una relación de varios a uno que requiera una regla NAT por cada VIP de usuario de todos los puertos que ese usuario podría utilizar.

Algunas de las desventajas del uso de NAT para una VIP pública son:

- Sobrecarga al administrar las reglas de firewall, ya que los usuarios controlan sus propios puntos de conexión y sus propias reglas de publicación en la pila de redes definidas por software. Los usuarios deben ponerse en contacto con el operador de MDC para que se publiquen sus VIP y para actualizar la lista de puertos.
...
En una implementación perimetral, MDC se implementa directamente detrás del firewall o el enrutador perimetral. En estos casos, se admite que el firewall esté encima del borde (escenario 1), donde admite configuraciones de firewall activo/activo y activo/pasivo. También puede actuar como el dispositivo de borde (escenario 2), donde solo es compatible con la configuración de firewall activo/activo. El escenario 2 se basa en una ruta de acceso múltiple de igual costo con enrutamiento BGP o estático para la conmutación por error.
...
Las direcciones IP enrutables públicas se especifican para el grupo de VIP públicas de la red externa en el momento de la implementación. Por motivos de seguridad, las direcciones IP enrutables públicas *no* se recomiendan en ninguna otra red en un escenario perimetral. Este escenario permite que un usuario disfrute de la experiencia completa en la nube autocontrolada como si se tratara de una nube pública como Azure.
 ...
 ![Diagrama que muestra los escenarios de servidor perimetral de MDC.](media/network-introduction/edge-firewall-scenario-60.png) 
...
- **Zona segura**: red interna que usa direcciones IP enrutables corporativas o internas. La red segura se puede dividir. Puede tener acceso de salida a Internet a través de la NAT del firewall. Normalmente es accesible desde dentro del centro de datos a través de la red interna. Todas las redes de MDC deben residir en la zona segura, excepto el grupo de VIP públicas de la red externa.
- **Zona perimetral**: la red perimetral es el lugar en que se suelen implementar las aplicaciones externas o accesibles desde Internet, como los servidores web. Se suele supervisar desde un firewall para evitar ataques como DDoS e intrusión (acceso por parte de piratas informáticos) y permitir el tráfico entrante especificado de Internet. Solo debe residir en la zona DMZ el grupo de VIP públicas de redes externas de MDC.
- **Zona no segura**: red externa, Internet. *No* se recomienda implementar MDC en la zona no segura.

![Diagrama que muestra un escenario de firewall de red perimetral.](media/network-introduction/perimeter-network-firewall-scenario-50.png)
...
Antes de crear y configurar las puertas de enlace de VPN para MDC, revise las consideraciones de redes de MDC. Aprenderá cómo las configuraciones de MDC difieren de las de Azure.
...
### <a name="configure-a-vpn-gateway"></a>Configuración de una instancia de VPN Gateway
...
#### <a name="site-to-site-and-multisite-ipsecike-vpn-tunnel"></a>Sitio a sitio y multisitio (túnel VPN de IPsec/IKE)

##### <a name="site-to-site"></a>De sitio a sitio

Una conexión de puerta de enlace de VPN de sitio a sitio es una conexión a través de un túnel VPN IPsec/IKE (IKEv2). Este tipo de conexión requiere un dispositivo VPN que esté ubicado en el entorno local y tenga asignada una dirección IP pública. Este dispositivo no puede estar detrás de un NAT. Se pueden utilizar conexiones de sitio a sitio para las configuraciones híbridas y entre locales.

##### <a name="multisite"></a>Multisitio

Una conexión multisitio es una variación de la conexión de sitio a sitio. Puede crear más de una conexión VPN desde la puerta de enlace de red virtual. Normalmente, se conecta con varios sitios locales. Cuando trabaje con varias conexiones, debe usar un tipo de VPN basado en ruta (conocido como puerta de enlace dinámica al trabajar con redes virtuales clásicas). Como cada red virtual solo puede tener una puerta de enlace VPN, todas las conexiones a través de la puerta de enlace comparten el ancho de banda disponible.
...
MDC no admite la SKU de puerta de enlace de ultraalto rendimiento, que se utiliza exclusivamente con Azure ExpressRoute. Tenga en cuenta los puntos siguientes cuando seleccione la SKU:
...
Los escenarios de alta disponibilidad solo se pueden configurar en la SKU de conexión de puerta de enlace de alto rendimiento. A diferencia de Azure, que proporciona disponibilidad a través de las configuraciones activa/activa y activa/pasiva, MDC solo admite la configuración activa/pasiva.

#### <a name="failover"></a>Conmutación por error

Hay tres VM de infraestructura de puerta de enlace multiinquilino en MDC. Dos de estas VM están en modo activo. La tercera está en modo redundante. Las VM activas admiten la creación de conexiones VPN en ellas. La VM redundante solo acepta conexiones VPN si se produce una conmutación por error. Si una VM de puerta de enlace activa no está disponible, la conexión VPN conmuta por error a la VM redundante tras un breve período (unos segundos) de pérdida de conexión.
...
| Tipo de puerta de enlace | Rendimiento de VPN Gateway (1) | Túneles IPsec máximos de VPN Gateway (2) |
|--------------|----------------------------|---------------------------------------|
...
>[!IMPORTANT]
> Solo se puede crear una conexión VPN de sitio a sitio entre dos implementaciones de MDC. Esta restricción se debe a una limitación en la plataforma que solo permite una conexión VPN a la misma dirección IP. Dado que MDC utiliza la puerta de enlace multiinquilino que usa una sola dirección IP pública para todas las puertas de enlace de la VPN del sistema de MDC, no puede haber más de una conexión VPN entre dos sistemas de MDC.
>
> Esta limitación se aplica también a la existencia de más de una conexión VPN de sitio a sitio a cualquier puerta de enlace de VPN que use una única dirección IP. MDC no permite que se cree más de un recurso de puerta de enlace de red local con la misma dirección IP.

### <a name="ipsecike-parameters"></a>Parámetros de IPsec/IKE

Cuando se configura una conexión VPN en MDC, es preciso configurar la conexión en ambos extremos. Si está configurando una conexión VPN entre MDC y un dispositivo de hardware, ese dispositivo podría solicitarle configuración adicional. Por ejemplo, el dispositivo puede solicitar un conmutador o un enrutador que actúe como puerta de enlace de VPN.

A diferencia de Azure, que admite varias ofertas como iniciador y respondedor, MDC admite solo una de forma predeterminada. Si tiene que usar distintas configuraciones IPsec/IKE para trabajar con el dispositivo VPN, hay más parámetros disponibles para configurar la conexión manualmente.
...
| Cifrado y algoritmos hash | AES256, SHA384 |
...
|--------------|-----------|
| Versión de IKE | IKEv2 |
| Cifrado y algoritmos hash (cifrado) | GCMAES256 |
| Cifrado y algoritmos hash (autenticación) | GCMAES256 |
| Vigencia de SA (tiempo) | 27 000 segundos |
| Vigencia de SA (kilobytes) | 33 553 408 |
| Confidencialidad directa perfecta (PFS) | ECP384 |
| Detección de elementos del mismo nivel inactivos | Compatible |
...
- La directiva de IPsec o IKE solo funciona en las SKU de puerta de enlace estándar y de alto rendimiento (basadas en rutas).
...
1. Cree una puerta de enlace de red local para la conexión entre locales.
1. Cree una directiva de IPsec o IKE con los algoritmos y parámetros seleccionados.
1. Cree una conexión de IPsec con la directiva de IPsec o IKE.
1. Agregue, actualice o quite una directiva de IPsec o IKE para una conexión existente.
...
### <a name="connect-mdc-to-azure-by-using-azure-expressroute"></a>Conexión de MDC a Azure mediante Azure ExpressRoute
...
- Conocimientos prácticos de Azure.
- Conocimientos básicos sobre MDC.
...
- Tener un circuito ExpressRoute aprovisionado a través de un proveedor de conectividad.
- Tener una suscripción a Azure para crear un circuito ExpressRoute y redes virtuales en Azure.
- Tener un enrutador que debe:
  - admitir conexiones VPN de sitio a sitio entre su interfaz LAN y la puerta de enlace multiinquilino de Azure Stack;
  - admitir la creación de varias instancias de reenvío y enrutamiento virtual si hay más de un inquilino en la implementación de MDC.
- Tener un enrutador que tenga:
  - Un puerto WAN conectado al circuito ExpressRoute.
  - un puerto LAN conectado a la puerta de enlace multiinquilino de MDC.

#### <a name="expressroute-network-architecture"></a>Arquitectura de red de ExpressRoute

En el diagrama siguiente se muestran los entornos de MDC y Azure después de finalizar la configuración de ExpressRoute mediante los ejemplos de este artículo.

 ![Diagrama que muestra la arquitectura de red de ExpressRoute.](media/network-introduction/expressroute-network-architecture-60.png)
